...
from cnn.chess.components.cnn.modules.chess_attention import ChessMultiStageAttention
...
class RegressionChessCNNv1(nn.Module):
    """
    ...
    def __init__(
        ...
        super(RegressionChessCNNv1, self).__init__()
        ...
        # Initialize 5-stage attention system
        self.multi_attention = ChessMultiStageAttention()
        ...
        with torch.no_grad():
            ...
            self.fc = nn.Linear(fc_input_size, 64 * 64)
    ...
    def _forward_features(self, x):
        ...
        x = self.initial_conv(x)
        x = self.multi_attention(x, stage=1)  # ATTENTION LAYER 2

        # Stage 3: Dense processing with mid-level attention
        x = self.dense_block1(x)
        x = self.transition1(x)  # Downsampling to 4×4
        x = self.multi_attention(x, stage=2)  # ATTENTION LAYER 3

        # Stage 4: SE processing with late attention
        identity = x
        x = self.se_block(x)
        x = self.stochastic_depth(x)
        x = x + identity
        x = self.multi_attention(x, stage=3)  # ATTENTION LAYER 4

        # Stage 5: Final spatial attention + self-attention
        x = self.spatial_attn(x)
        x = self.multi_attention(x, stage=4)  # ATTENTION LAYER 5
    ...
    def get_move_probabilities(self, x):
        """Get move probabilities directly from regression output."""
        probabilities = self.forward(x)  # Already probabilities, no softmax needed
        probabilities = probabilities.view(probabilities.size(0), 64, 64)
