...
class ChessDataProcessor:
    def __init__(self, pgn_file: str = "chess_games_10k.pgn"):
        ...
        # Initialize tokenizer for TinyLlama-1.1B-Chat-v1.0
    ...
    def game_to_training_examples(self, game: chess.pgn.Game) -> List[Dict]:
        ...
        for i in range(1 if result == "black" else 0, len(moves), 1 if result == "draw" else 2):  # Every 5 moves up to move 50
            ...
            if i < len(moves) and moves[i] in temp_board.legal_moves:
                ...
                conversation = self.create_move_prediction_conversation(
                    game_history, next_move, temp_board
                )
    ...
    def create_move_prediction_conversation(self, game_history: List[str],
                                            next_move: str, board: chess.Board) -> Dict:
        ...
        user_prompt = f"""I'm analyzing a chess game.
...
This move {"addresses the check" if is_check else "continues the game's development"}."""
    ...
    def process_all_games(self, games, start, batch_length, train_split: float = 0.8) -> Tuple[List[Dict], List[Dict]]:
    ...
    def save_training_data(self,
                           ...
                           val_data: List[Dict],
                           iteration,
                           folder: str = "./training_data/",
                           ):
        """Save training data in JSONL format for Mistral fine-tuning"""

        # Save training data
        with open(f"{folder}chess_train_{iteration}.jsonl", 'w', encoding='utf-8') as f:
            for example in train_data:
                f.write(json.dumps(example) + '\n')

        # Save validation data
        with open(f"{folder}chess_val_{iteration}.jsonl", 'w', encoding='utf-8') as f:
            for example in val_data:
                f.write(json.dumps(example) + '\n')

        print(f"Saved {len(train_data)} training examples to chess_train_{iteration}.jsonl")
        print(f"Saved {len(val_data)} validation examples to chess_val_{iteration}.jsonl")
...
What should be the next move?"""
    ...
    def prepare_data(self):
        ...
        if not Path(self.pgn_file).exists():
            raise FileNotFoundError(f"PGN file {self.pgn_file} not found. NOT Creating sample data...")
        ...
        total_train_data = []
        total_val_data = []
        for i in range(0, len(games), batch_length):
            train_data, val_data = self.process_all_games(games, i, batch_length)
            total_train_data = total_train_data + train_data
            total_val_data = total_val_data + val_data
            self.save_training_data(train_data, val_data, iteration)
            iteration = iteration + 1

        print("Data preparation complete!")
        print(f"Training examples: {len(total_train_data)}")
        print(f"Validation examples: {len(total_val_data)}")

        return total_train_data, total_val_data
